...
//모델파일 사용안함
//히스토그램 기반 메트릭스 + 유클리디안거리
//근데 그냥 히스토그램이랑 결과값이 같음 
...
return similarity;
        ...
        // 두 벡터 간의 유클리디안 거리 계산 함수
const calculateEuclideanDistance = (vector1, vector2) => {
         if (vector1.length !== vector2.length) {
           throw new Error('두 벡터의 길이가 같아야 합니다.');
         }
...
// 이미지 간의 유사성 메트릭스 계산 함수 (히스토그램 + 유클리디안 거리)
         ...
         try {
           ...
           for (const imagePath of imagePaths) {
             try {
               ...
               const histogramSimilarity = calculateHistogramBasedSimilarity(inputHistogram, folderHistogram);
       
               // 유클리디안 거리 계산
               const euclideanDistance = calculateEuclideanDistance(inputHistogram, folderHistogram);
       
               // 히스토그램 유사성과 유클리디안 거리를 합친 유사성 메트릭스
               const combinedSimilarity = 0.5 * histogramSimilarity + 0.5 * (1 / (1 + euclideanDistance));
       
               console.log('Combined Similarity for', imagePath, ':', combinedSimilarity); // 유사성 메트릭스 출력
       
               similarityMatrix.push({ imagePath, similarity: combinedSimilarity });
             } catch (imageError) {
