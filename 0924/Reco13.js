...
//모델파일 사용안함
// 히스토그램 오버랩, 히스토그램 기반 메트릭스
// 히스토그램 계산을 위해 이미지 데이터를 사용하고, 
//히스토그램 오버랩은 각 이미지 간의 픽셀값 분포를 비교하여 계산됩니다.
  ...
  try {
    ...
    if (classIndex !== -1) {
      ...
      if (imagePathsResponse.ok) {
        ...
        const topSimilarImages=await calculateHistogramBasedMatrix (updatedImagePaths);
...
// 히스토그램 오버랩 계산 함수
const calculateHistogramOverlap = (histogramA, histogramB) => {
         if (histogramA.length !== histogramB.length) {
           console.error('Histograms must have the same length.');
           return 0;
         }
       
         let overlap = 0;
         for (let i = 0; i < histogramA.length; i++) {
           overlap += Math.min(histogramA[i], histogramB[i]);
         }
...
// 히스토그램 기반 메트릭스 계산 함수
const calculateHistogramBasedMatrix = (imagePaths) => {
const similarityMatrix = [];

// 입력 이미지의 특성 추출
const inputImageData = getImageData(imageFile);
const inputHistogram = calculateHistogram(inputImageData);

for (const imagePath of imagePaths) {
         try {
         const folderImageData = getImageDataFromPath(imagePath);
         const folderHistogram = calculateHistogram(folderImageData);

         // 히스토그램 오버랩 계산
         const overlap = calculateHistogramOverlap(inputHistogram, folderHistogram);

         // 유사성 메트릭스에 추가
         similarityMatrix.push({ imagePath, similarity: overlap });
         } catch (imageError) {
