...
// 이미지 간의 유사성 메트릭스 계산 함수 (히스토그램 + 코사인 유사도)
const calculateImageSimilarityMatrix = async (imagePaths) => {
         try {
           ...
           for (const imagePath of imagePaths) {
             try {
               ...
               // 코사인 유사도 계산
               const cosineSimilarity = calculateCosineSimilarity(inputHistogram, folderHistogram);
       
               // 히스토그램 유사성과 코사인 유사도를 조합하여 유사성 메트릭스 계산
               const combinedSimilarity = 0.5 * histogramSimilarity + 0.5 * cosineSimilarity;
       ...
       const calculateCosineSimilarity = (vectorA, vectorB) => {
         ...
         if (magnitudeA === 0 || magnitudeB === 0) {
           return 0; // 분모가 0인 경우 0으로 처리하여 에러 방지
         }
