...
const calculateCosineSimilarity = async () => {
  try {
    ...
    // 사용자 업로드 이미지의 벡터 계산
    const userImageTensor = tf.tensor(pretrainedPrediction);
    ...
    for (const imagePath of imagePathsInFolder) {
      ...
      const cosineSimilarity = calculateCosineSimilarityBetweenTensors(userImageTensor, folderImageTensor);
    ...
    console.log("Top similar images:", topSimilarImages);

    // 여기에서 유사한 이미지를 사용자에게 보여줄 수 있습니다.
