...
function CosineExs() {
  ...
  const handleCosineCalculation = async () => {
         try {
           ...
           const topSimilarImages = await calculateCosineSimilarity(inputImageTensor);
       ...
       const calculateCosineSimilarity = async (inputImageTensor) => {
         try {
           ...
           for (const imagePath of imagePaths) {
             ...
             const folderNormalizedImg = folderExpandedImg.div(255.0);
                // 중간 단계의 값을 콘솔에 출력
                  console.log('추가_Input Image Tensor:', inputImageTensor);
                  console.log('추가_Folder Normalized Image Tensor:', folderNormalizedImg);

             const cosineSimilarity = calculateCosineSimilarityBetweenTensors(
               inputImageTensor,
               folderNormalizedImg
             );
             console.log('추가_Cosine Similarity:', cosineSimilarity);
  ...
  const handleImageFileChange = async (event) => {
    ...
    // 이미지를 낮은 해상도로 리사이즈합니다.
    const resizedImage = tf.image.resizeBilinear(inputImageTensor, [250, 250]);
