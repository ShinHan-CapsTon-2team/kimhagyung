...
//모델파일 사용함 
//맨해튼거리 
// 두 이미지 간의 픽셀 값 차이의 절대값을 합한 거리를 계산하는 방법입니다.
// 유클리디안 거리와 달리 대각선 방향의 이동을 고려하지 않습니다.
...
// 두 텐서 간의 맨해튼 거리 계산
const calculateManhattanDistanceBetweenTensors = (tensorA, tensorB) => {
         const absoluteDifferences = tf.abs(tf.sub(tensorA, tensorB));
         const sumAbsoluteDifferences = tf.sum(absoluteDifferences);
         const manhattanDistance = sumAbsoluteDifferences.arraySync();
         return manhattanDistance;
       };
       
       // 이미지 간의 유사성 메트릭스 계산 함수 (맨해튼 거리 사용)
         ...
         try {
           ...
           for (const imagePath of imagePaths) {
             try {
               ...
               // 이미지 간의 유사성 메트릭스 계산 (맨해튼 거리)
               const similarity = calculateManhattanDistanceBetweenTensors(inputImageTensor, folderImageTensor);
       
               console.log('Manhattan Distance for', imagePath, ':', similarity); // 유사성 메트릭스 출력
           ...
           similarityMatrix.sort((a, b) => a.similarity - b.similarity);
