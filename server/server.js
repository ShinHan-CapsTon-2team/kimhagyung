...
app.post('/api/post', upload.single('image'), (req, res) => {
  ...
  const query = 'INSERT INTO your_post (title, description, image_url, category, name, profile) VALUES (?, ?, ?, ?, ?, ?)';
  const values = [data.title, data.description, req.file.path, data.category, data.name, data.profile];
